...
func (s *Service) EditMessage(
	...
	replacement := newController(id, conversation, generation, source.harness, provider, s.store, s.activity, s.log, s.newID, s.now, s.onAccountChanged, s.onCodexCapacityChanged, s.onInputEscalation, s.onApprovalEscalation)
...
func (s *Service) restoreClosedSourceController(
	...
	replacement := newController(
		id, conversation, generation, source.harness, provider, s.store, s.activity, s.log, s.newID, s.now, s.onAccountChanged, s.onCodexCapacityChanged, s.onInputEscalation, s.onApprovalEscalation)
