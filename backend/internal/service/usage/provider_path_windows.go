...
// file or directory, following junctions the way Windows itself does when
// opening a handle — not by manually walking reparse points the way
...
// though ordinary file APIs open the exact same path without issue.
// GetFinalPathNameByHandle asks the OS directly, from an open handle, so it
// resolves exactly the way os.Open/os.Stat already do. Any failure is
// returned rather than degraded to the unresolved literal path: callers that
// use this for allowed-root containment must stay fail-closed.
...
// trimExtendedLengthPrefix strips the \\?\ extended-length prefix that
