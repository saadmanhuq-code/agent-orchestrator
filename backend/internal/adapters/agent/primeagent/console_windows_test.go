...
func TestMain(m *testing.M) {
	if os.Getenv("AO_TEST_BACKGROUND_AGENT_CONSOLE") == "primeagent" {
...
func TestBackgroundCommandHasNoConsole(t *testing.T) {
	...
	t.Setenv("AO_TEST_BACKGROUND_AGENT_CONSOLE", "primeagent")
	if _, err := runPrimeCommand(context.Background(), binary, "", nil, "--version"); err != nil {
		t.Fatal(err)
	}
}
