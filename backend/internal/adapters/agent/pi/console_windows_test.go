...
func TestMain(m *testing.M) {
	if os.Getenv("AO_TEST_BACKGROUND_AGENT_CONSOLE") == "pi" {
...
func TestBackgroundCommandHasNoConsole(t *testing.T) {
	...
	t.Setenv("AO_TEST_BACKGROUND_AGENT_CONSOLE", "pi")
	p := &Plugin{resolvedBinary: binary}
	if _, err := p.piAgentSettledSupported(context.Background()); err != nil {
		t.Fatal(err)
	}
}
