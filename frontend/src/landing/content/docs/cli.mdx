...
| Command | Purpose |
| --- | --- |
| `ao pr merge <pr-ref>` | Request a merge through AO's GitHub action engine. |
...
`ao pr merge` requires `--expected-head-sha`; the daemon refuses the merge if the PR's head has moved. A bare PR/MR number is accepted only when AO can resolve one identified project—pass `--project` to make that explicit, or use `AO_PROJECT_ID`, the session's project, or the current registered repo. A full PR/MR URL is unambiguous on its own and needs no project:

```bash
ao pr merge 123 --project <project-id> --expected-head-sha <40-character-commit-sha>
```
