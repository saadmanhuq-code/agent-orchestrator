...
Reuse the session record already read while preparing the outbound message, preserving other harnesses' session-read ordering and permission-race behavior.
...
- machine: `go -C backend test ./internal/session_manager -run '^TestSend' -count=1`
